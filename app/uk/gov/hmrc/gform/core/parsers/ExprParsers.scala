...
object ExprParsers {
  ...
  lazy val expr: Parser[FormCtx] = "$" ~ contextField ~ "" ^^ { (loc, _, field, _) =>
    field
  }

  lazy val contextField: Parser[FormCtx] = wordOnly ^^ { (loc, fn) =>
    FormCtx(fn)
  }
  ...
  lazy val anyDigitConst: Parser[Expr] = (""" *[\d.][ \d.,]*""".r ^^ { (loc, str) =>
    Constant(str)
  })
